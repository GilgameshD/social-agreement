...
LastEditTime: 2023-08-23 13:25:57
Description: 
'''
...
class GroupDataset(IterableDataset):
    def __init__(self, dataset: AbstractDataset, split: str):
        ...
        assert split in {'train', 'val', 'match'}
        ...
        elif self.split == 'match':
            self.fetch_f = self.dataset.fetch_match_example
        else:
...
def train(config):
    ...
    optim = torch.optim.AdamW(model.parameters(), lr=train_cfg['lr'], weight_decay=train_cfg['weight_decay'], betas=train_cfg['betas'])
    lr_schedule = torch.optim.lr_scheduler.LambdaLR(optim, lr_lambda=lambda s: min(s / train_cfg['warmup_steps'], 1))
    step = 0
    for x, y in tqdm(train_dataloader):
        loss, logs = model.get_loss(x.to(device), y.to(device), match_data)
        optim.zero_grad()
        loss.backward()
        optim.step()
        lr_schedule.step()
            ...
            with torch.no_grad():
                ...
                for i, (val_x, val_y) in tqdm(enumerate(val_dataloader)):
                    if i >= train_cfg['eval_batches']:
                        break
                    _, val_logs = model.get_loss(val_x.to(device), val_y.to(device), match_data)
                    all_val_logs.append(val_logs)
            out_log = {'val': combine_logs(all_val_logs), 'train': combine_logs([logs]), 'lr': float(lr_schedule.get_last_lr()[0])}
            print(out_log)
            if wandb_cfg['use_wandb']:
                wandb.log(out_log)
            model.train()
