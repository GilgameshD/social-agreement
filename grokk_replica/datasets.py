...
LastEditTime: 2023-08-23 13:04:02
Description: 
'''
...
class AbstractDataset(abc.ABC):
    def __init__(self, group_elements1: Set, group_elements2: Set, frac_train: float):
        ...
        # seperate a match pair from train_pairs
        frac_match = 0.1
        self.match_pairs = self.train_pairs[:int(len(self.train_pairs)*frac_match)]
        self.train_pairs = self.train_pairs[int(len(self.train_pairs)*frac_match):]
    ...
    def fetch_match_example(self):
        idx = random.choice(self.match_pairs)
        return self.fetch_example(idx)
